...
@Component({
  selector: 'app-gallery-notice',
  templateUrl: './gallery-notice.component.html',
  animations: [fadeIn,],
})
export class GalleryNoticeComponent implements OnInit {
  ...
  async loadItems() {
    ...
    try {
      ...
      this.qp.noticeLocation = Cd.NoticeLocation.gallery;
      // this.qp.set('siteIdx', this._api.siteIdx);
      console.log(ftag, 'qp=', this.qp.qpBody);
      this.isLoading = true;
  ...
  onClickAdd() {
    this._api.navigate('/p/h/notice/add', {
      queryParams: {
        galleryIdx: this.qp.galleryIdx,
      }
    });
  }
